...
fun main() {
  ...
  val pets = listOf("cat", "dog", "canary", "cow", "fish")
  val filteredPets = pets.filter(::startsWithC)
  println(filteredPets)

  // val filteredPets2 = pets.filter() { pet -> pet.startsWith('c') }
  val filteredPets2 = pets.filter() { it.startsWith('c') }
  println(filteredPets2)
}
